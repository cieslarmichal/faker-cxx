...
bool checkTokenFormat(const std::string& bio)
{
    ...
    {
        if (matches.size() >= 1 && std::find(bioPart.begin(), bioPart.end(), matches.str(0)) != bioPart.end())
            return true;
    }

    if (std::regex_match(bio, matches, secondRegex))
    {
        if (matches.size() >= 3 && std::find(bioPart.begin(), bioPart.end(), matches.str(1)) != bioPart.end() &&
            std::find(bioPart.begin(), bioPart.end(), matches.str(2)) != bioPart.end())
            return true;
    }

    if (std::regex_match(bio, matches, thirdRegex))
    {
        if (matches.size() >= 4 && std::find(bioPart.begin(), bioPart.end(), matches.str(1)) != bioPart.end() &&
            std::find(bioPart.begin(), bioPart.end(), matches.str(2)) != bioPart.end() &&
            std::find(bioPart.begin(), bioPart.end(), matches.str(3)) != bioPart.end())
            return true;
    }

    if (std::regex_match(bio, matches, fourthRegex))
    {
        if (matches.size() >= 5 && std::find(bioPart.begin(), bioPart.end(), matches.str(1)) != bioPart.end() &&
            std::find(bioPart.begin(), bioPart.end(), matches.str(2)) != bioPart.end() &&
            std::find(bioPart.begin(), bioPart.end(), matches.str(3)) != bioPart.end() &&
            Internet::checkIfEmojiIsValid(matches.str(4)))
            return true;
    }

    if (std::regex_match(bio, matches, fifthRegex))
    {
        if (matches.size() >= 3 && std::find(nouns.begin(), nouns.end(), matches.str(1)) != nouns.end() &&
            std::find(bioSupporter.begin(), bioSupporter.end(), matches.str(2)) != bioSupporter.end())
            return true;
    }

    if (std::regex_match(bio, matches, sixthRegex))
    {
        if (matches.size() >= 4 && std::find(nouns.begin(), nouns.end(), matches.str(1)) != nouns.end() &&
            std::find(bioSupporter.begin(), bioSupporter.end(), matches.str(2)) != bioSupporter.end() &&
            Internet::checkIfEmojiIsValid(matches.str(3)))
            return true;
    }
    ...
    {
        if (matches.size() >= 5 && std::find(nouns.begin(), nouns.end(), matches.str(1)) != nouns.end() &&
            std::find(bioSupporter.begin(), bioSupporter.end(), matches.str(2)) != bioSupporter.end() &&
            std::find(bioPart.begin(), bioPart.end(), matches.str(3)) != bioPart.end() &&
            Internet::checkIfEmojiIsValid(matches.str(4)))
            return true;
