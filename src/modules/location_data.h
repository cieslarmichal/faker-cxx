...
const auto iranCities = std::to_array<std::string_view>({
    // clang-format off
    "آب بر",
    "آبادان",
    "آباده",
...
const auto iranStates = std::to_array<std::string_view>({
    // clang-format off
    "آذربایجان شرقی",
    "آذربایجان غربی",
    "اردبیل",
...
const auto iranStreetNames = std::to_array<std::string_view>({
    // clang-format off
    "ولیعصر",
    "انقلاب",
    "آزادی",
...
const auto iranStreetPrefixes = std::to_array<std::string_view>({
    // clang-format off
    "خیابان",
    "بلوار",
    // clang-format on
...
const std::string_view iranZipCodeFormat{"##########"};

const auto iranAddressFormats = std::to_array<std::string_view>({
    // clang-format off
    "{street}، {buildingNumber}",
    "{street}، {buildingNumber}، {secondaryAddress}",
    // clang-format on
...
const auto iranSecondaryAddressFormats = std::to_array<std::string_view>({
    // clang-format off
    "واحد #",
    "واحد ##",
    "طبقه #",
...
const auto iranBuildingNumberFormats = std::to_array<std::string_view>({
    // clang-format off
    "پلاک #",
    "پلاک ##",
    "پلاک ###",
...
const auto iranStreetFormats = std::to_array<std::string_view>({
    // clang-format off
    "{streetPrefix} {streetName}",
    "میدان {streetName}، خیابان {streetName}",
    "میدان {streetName}، بلوار {streetName}",
    "اتوبان {streetName}، خیابان {streetName}",
    "بزرگراه {streetName}، خیابان {streetName}",
    // clang-format on
});

const auto iranCityFormats = std::to_array<std::string_view>({
    "{cityName}",
});

const CountryAddressesInfo iranAddresses{
    iranZipCodeFormat,
    (iranAddressFormats),
    (iranSecondaryAddressFormats),
    (iranStreetFormats),
    (iranStreetPrefixes),
    (iranStreetNames),
    {},
    (iranBuildingNumberFormats),
    (iranCityFormats),
    {},
    (iranCities),
    {},
...
const auto romaniaCities = std::to_array<std::string_view>({
    "Abrud",
	"Adjud",
	"Agnita",
	"Aiud",
...
const auto romaniaStates = std::to_array<std::string_view>({
        "Alba",
	"Argeș",
	"Arad",
	"București",
...
const auto romaniaCityFormats = std::to_array<std::string_view>({
    "{cityName}",
});

const std::string_view romaniaZipCodeFormat{"######"};

const auto romaniaAddressFormats =
    std::to_array<std::string_view>({
    "{street} {buildingNumber}",
    "{street} {buildingNumber}, {secondaryAddress}"
});

const auto romaniaSecondaryAddressFormats = std::to_array<std::string_view>({
    "Apartamentul #",
    "Bloc #",
    "Casa #",
});

const auto romaniaBuildingNumberFormats = std::to_array<std::string_view>({
    "#",
    "##",
    "###"
});

const auto romaniaStreetPrefixes = std::to_array<std::string_view>({
    "aleea",
    "bulevardul",
    "calea",
    "centura",
...
const auto romaniaStreetNames = std::to_array<std::string_view>({
        "14 Iulie",
	"21 Decembrie 1989",
	"Albăstrelelor",
	"Aleea Coralului",
...
const auto romaniaStreetFormats = std::to_array<std::string_view>({
    "{streetPrefix} {streetName}"
});

const CountryAddressesInfo romaniaAddresses{
    romaniaZipCodeFormat,
...
const auto netherlandsCities = std::to_array<std::string_view>({
    "Amsterdam", "Rotterdam", "The Hague", "Utrecht", "Eindhoven", "Groningen", "Tilburg", "Almere", "Breda",
    "Nijmegen"
});

const auto netherlandsStates = std::to_array<std::string_view>({
    "North Holland", "South Holland", "Utrecht", "North Brabant", "Gelderland", "Groningen", "Overijssel",
    "Limburg", "Friesland", "Zeeland"
});

const auto netherlandsStreetNames = std::to_array<std::string_view>({
    "Damrak", "Kalverstraat", "Prinsengracht", "Herengracht", "Nieuwendijk", "Wilhelminaplein", "Vondelstraat",
    "Coolsingel", "Lange Voorhout", "Oudegracht"
});

const std::string_view netherlandsZipCodeFormat{"#### __"};

const auto netherlandsAddressFormats = std::to_array<std::string_view>({
    "{street} {buildingNumber}"
});

const auto netherlandsBuildingNumberFormats = std::to_array<std::string_view>({
    "#", "##", "###"
});

const auto netherlandsSecondaryAddressFormats = std::to_array<std::string_view>({
    "unit #", "unit ##"
});

const auto netherlandsStreetFormats = std::to_array<std::string_view>({
    "{streetName}"
});

const auto netherlandsCityFormats = std::to_array<std::string_view>({
    "{cityName}"
});

const CountryAddressesInfo netherlandsAddresses{
    netherlandsZipCodeFormat,
    (netherlandsAddressFormats),
    (netherlandsSecondaryAddressFormats),
    (netherlandsStreetFormats),
    {},                             // no street prefixes
    (netherlandsStreetNames),
    {},                             // no street suffixes
    (netherlandsBuildingNumberFormats),
    (netherlandsCityFormats),
    {},                             // no city prefixes
    (netherlandsCities),
    {},                             // no city suffixes
    (netherlandsStates)
