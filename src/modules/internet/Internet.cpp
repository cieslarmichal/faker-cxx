                               ...
                               Country country)
{
    ...
    switch (faker::number::integer<int>(2))
    {
    case 0:
        username = FormatHelper::format("{}{}{}", firstName, lastName, faker::number::integer<int>(999));
        break;
    case 1:
    ...
    case 2:
        username =
            FormatHelper::format("{}{}{}{}", firstName, Helper::arrayElement(std::vector<std::string>{".", "_", ""}),
                                 lastName, faker::number::integer<int>(99));
        break;
    }
...
std::string Internet::ipv4(const IPv4Class& ipv4class)
{
    ...
    sectors[3] = faker::number::integer<unsigned int>(ipv4SectorUpperBound);
    sectors[2] = faker::number::integer<unsigned int>(ipv4SectorUpperBound);

    switch (ipv4class)
    {
    case IPv4Class::A:
    {
        sectors[1] = faker::number::integer<unsigned int>(ipv4SectorUpperBound);
        sectors[0] = ipv4ClassAFirstSector;
        break;
    }
    case IPv4Class::B:
    {
        sectors[1] = faker::number::integer<unsigned int>(ipv4ClassBSecondSectorLowerBound, ipv4ClassBSecondSectorUpperBound);
                           ...
                           const std::array<unsigned int, 4>& generationMask)
{
    ...
    {
        sectors[i] = (~generationMask[i]) & faker::number::integer<unsigned int>(ipv4SectorUpperBound);
...
{
    return faker::number::integer(65535u);
}
...
std::string Internet::anonymousUsername(unsigned maxLength)
{
    ...
    const std::integral auto adjectiveLength = faker::number::integer<unsigned>(3, 1 + maxLength / 2);
