...
{
    switch (faker::number::integer(1, 3))
    {
    case 1:
    ...
    default:
        return FormatHelper::format("{}-{}-{}-{}", faker::number::integer(unsigned(1), maxIssueNum), word::verb(),
...
std::string commitDate(unsigned years)
{
    ...
    int timeZone = faker::number::integer(0, 12);

    std::string timeZoneString;

    if (faker::number::integer(0, 1))
    {
        timeZoneString += "-";
    }
    else
...
{
    switch (faker::number::integer(1, 4))
    {
    case 1:
        return FormatHelper::format("{} {}", word::verb(), word::noun());
    case 2:
