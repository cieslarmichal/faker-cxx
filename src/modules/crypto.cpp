...
namespace {
    ...
    private:
        ...
        constexpr static uint32_t k[64] = { 0xd76aa478, 0xe8c7b756, 0x242070db, 0xc1bdceee,
...
namespace {
    ...
    std::array<uint8_t, 16> md5_hash::compute(const std::string& message)
    {
        ...
        std::array<uint8_t, 4> bytes
            = md5_hash::uint32_to_4_bytes(static_cast<uint32_t>(message.size() * 8));
        ...
        for (size_t i = 0; i < new_len; i += (512 / 8)) {
            ...
            for (size_t j = 0; j < 64; j++) {
                ...
                b = b
                    + md5_hash::rotate_left(
                        (a + f + md5_hash::k[j] + w[g]), static_cast<int32_t>(md5_hash::r[j]));
                a = temp;
            }
